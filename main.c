...
¸ß¼¶PWM¶¨Ê±Æ÷ PWM5,PWM6,PWM7,PWM8 Ã¿¸öÍ¨µÀ¶¼¿É¶ÀÁ¢ÊµÏÖPWMÊä³ö.

4¸öÍ¨µÀPWM¸ù¾İĞèÒªÉèÖÃ¶ÔÓ¦Êä³ö¿Ú£¬¿ÉÍ¨¹ıÊ¾²¨Æ÷¹Û²ìÊä³öµÄĞÅºÅ.

PWMÖÜÆÚºÍÕ¼¿Õ±È¿ÉÒÔ×Ô¶¨ÒåÉèÖÃ£¬×î¸ß¿É´ï65535.
...
/* ===== Ñ­¼£µ÷²ÎÇø£º5Â·´«¸ĞÆ÷£¬ºÚÏßÊä³öÎª1 =====
   ÓÅÏÈ¼¶£ºÂí´ïÏìÓ¦¼°Ê± > ÔËĞĞÎÈ¶¨ > Ñ­¼£ËÙ¶ÈºÍ×¼È·ĞÔ¡£
   µ÷²Î½¨Òé£º
   1. ÏìÓ¦²»¼°Ê±£ºÈ·ÈÏ CTRL_DIV_1MS Îª10£¬²»Òª¼ÓÈë¹Ì¶¨×ªÏò±£³ÖÊ±¼ä¡£
   2. µç»ú¿¨ËÀ/Ö»Ïì²»×ª£ºÌá¸ß MIN_RUN_PWM »ò MICRO_INNER_PWM¡£
   3. Ğ¡Íä³å³öÈ¥£º½µµÍ MICRO_OUTER_PWM »ò SHARP_FORWARD_PWM¡£
   4. Ö±½Ç×ª²»¹ı£ºÌá¸ß SHARP_FORWARD_PWM£¬²»ÒªÔö¼Ó¹Ì¶¨±£³ÖÊ±¼ä¡£
*/
#define LINE_ON             1       /* ºÚÏßÓĞĞ§µçÆ½£ººÚÏß=1£¬°×µ×=0 */
#define CTRL_DIV_1MS        10      /* Timer2Îª100usÖĞ¶Ï£¬10´ÎÖ´ĞĞÒ»´ÎÑ­¼££¬Ô¼1ms */

#define BASE_FAST           85      /* 00100Ö±ĞĞËÙ¶È */
#define BASE_NODE           78      /* Ê®×ÖÅĞ¶¨ºóÈÔÑ¹ÔÚÏßÉÏÊ±µÄÖ±ĞĞËÙ¶È */
#define MAX_PWM             92      /* PWM×î´óÏŞ·ù */
#define MIN_RUN_PWM         60      /* ·ÇÍ£³µÕı×ªÊ±×îµÍPWM */
#define LEFT_TRIM           2       /* å·¦è½®å¹³è¡¡åç½®,èŒƒå›´-10~10 */
#define RIGHT_TRIM          4       /* å³è½®å¹³è¡¡åç½®,èŒƒå›´-10~10 */
#define LOST_TURN_PWM       80      /* ³ÖĞøÈ«°×ºó°´ÉÏ´Î·½ÏòÕÒÏßËÙ¶È */

#define HOLD_CURRENT_MS     110     /* Î¬³Öµ±Ç°µç»úÊä³ö */
#define CROSS_HOLD_MS       80      /* Ê®×ÖÂ·¿ÚÖ±ĞĞ±£³ÖÊ±¼ä */
#define UTURN_STOP_MS       500     /* ÓÒµôÍ·Ç°Í£³µÊ±¼ä */

#define MICRO_OUTER_PWM     85      /* Î¢ĞÍĞŞÕıÍâ²àÂÖËÙ¶È */
#define MICRO_INNER_PWM     80      /* Î¢ĞÍĞŞÕıÄÚ²àÂÖËÙ¶È */
#define MICRO_TO_SHARP_TICKS 20     /* Ğ¡ĞŞºóÔÊĞí´óĞŞÇÀÕ¼µÄÊ±¼ä */
#define CENTER_SHARP_WINDOW_TICKS 15 /* ÖĞÏßºÍÍâ²à´«¸ĞÆ÷×éºÏ´¥·¢´óÍäµÄÊ±¼ä´°¿Ú */
#define MICRO_DIR_NONE       0
#define MICRO_DIR_LEFT      -1
#define MICRO_DIR_RIGHT      1

#define SHARP_FORWARD_PWM   75      /* ´óĞÍ×ªÍäÕı×ªÂÖËÙ¶È */
#define SHARP_REVERSE_PWM   90      /* ´óĞÍ×ªÍä·´×ªÂÖËÙ¶È */
#define SHARP_MIN_PIVOT_TICKS 50    /* ´óĞÍĞŞÕı×î¶ÌÇ¿×ªÊ±¼ä */


/* PID å¾ªè¿¹æ§åˆ¶å‚æ•° */
#define PID_KP              6      /* æ¯”ä¾‹ç³»æ•° */
#define PID_KI              0      /* ç§¯åˆ†ç³»æ•° (0=æš‚ä¸å¯ç”¨) */
#define PID_KD              15     /* å¾®åˆ†ç³»æ•°,æŠ‘åˆ¶éœ‡è¡ */
#define PID_INTEGRAL_MAX    30     /* ç§¯åˆ†é™å¹…,é˜²æ­¢ç§¯åˆ†é¥±å’Œ */
#define PID_OUTPUT_MAX      30     /* PIDå•ä¾§æœ€å¤§è¾“å‡ºé™å¹… */
#define UTURN_PWM           80      /* ÓÒµôÍ·×óÓÒÂÖµÈËÙ·´ÏòËÙ¶È */

#define LOST_CONFIRM_TICKS  60     /* Ô¤Áô£º120*1ms=120ms */
#define LOST_STRAIGHT_TICKS 30     /* ¶ÌÔİÈ«°×±£³ÖÉÏÒ»Êä³öÊ±¼ä */

#define MASK_ZUO2           0x10    /* 1ºÅ´«¸ĞÆ÷£º×î×ó */
#define MASK_ZUO1           0x08    /* 2ºÅ´«¸ĞÆ÷£º×óÄÚ */
#define MASK_ZHONG          0x04    /* 3ºÅ´«¸ĞÆ÷£ºÖĞ¼ä */
#define MASK_YOU1           0x02    /* 4ºÅ´«¸ĞÆ÷£ºÓÒÄÚ */
#define MASK_YOU2           0x01    /* 5ºÅ´«¸ĞÆ÷£º×îÓÒ */
...
void PWM_Left(int pwm)     //ÓÃ0~100±íÊ¾
{
	
	PWMB_Duty.PWM6_Duty = pwm*(PWM_Period/100);  //×¢ÒâPWM_Period´ËÊ±Îª100±¶Êı 2000
	
}

void PWM_Right(int pwm)     //ÓÃ0~100±íÊ¾
{
...
void PWM_Run(int pwm1,int pwm2)  //×óÓÒPWM
{
	pwm1 = LimitPwm(pwm1);
	pwm2 = LimitPwm(pwm2);
	PWM_Left(pwm1);
...
void Motor_RunSafe(int left_pwm, int right_pwm)
{
	/* Ö»ÓĞ0,0ÔÊĞíÍ£³µ£»ÆÕÍ¨Ñ­¼£Ê±×óÓÒÂÖ±£³Ö¸ßÓÚÆğ×ªPWM */
	if(left_pwm == 0 && right_pwm == 0)
	{
		Motor_RunSigned(0, 0);
		return;
...
u8 ReadTrackSensors(void)
{
	/* ¶ÁÈ¡5Â·Ñ­¼££¬°´ ×óÍâ/×óÄÚ/ÖĞ/ÓÒÄÚ/ÓÒÍâ ´ò°ü³ÉbitÑÚÂë */
	u8 mask;
	mask = 0;
...
void Track_NormalControl(u8 mask, u8 count)
{
	...
	/* ä¸­å¿ƒæœ€è¿‘ä¸¢å¤± + æœ€å¤–ä¾§è§¦å‘ -> æ€¥è½¬å¼¯ (PIDå…œåº•) */
	...
	/* x111x åå­—è·¯å£ -> ç›´è¡Œä¿æŒ,æ¸…é›¶ç§¯åˆ† */
	if(IsCrossMask(mask))
	{
		Track_ClearMicroWindow();
		pid_integral = 0;
	...
	/* 10001 ä¸¤ä¾§æœ€å¤–åŒæ—¶çœ‹åˆ° -> æŒ‰ä¸Šæ¬¡æ–¹å‘ */
	...
	/* === PID æ­£å¸¸å¾ªè¿¹ (å¤„ç†æ‰€æœ‰ä¸­é—´æƒ…å†µ) === */
...
{
	/* Timer2Ã¿100us½øÒ»´Î£¬ÕâÀï·ÖÆµµ½Ô¼1msÖ´ĞĞ¿ØÖÆ£¬ÓÅÏÈ±£Ö¤ÏìÓ¦¼°Ê± */
	ctrl_divider++;
